...
public class CombatEventHandler {
    ...
    public void onCriticalHit(CriticalHitEvent event) {
        ...
        if (heldItem.getItem() instanceof SwordItem) {
            // Mark this attack as NOT a critical hit
            event.setCriticalHit(false);

            // Also reset any critical damage multiplier to vanilla non-critical value (optional)
            event.setDamageMultiplier(1.0f);

            // If your NeoForge version exposes sweep-control you can tweak that too:
            // event.setDisableSweepAttack(true/false); // check if available in your API
        }
    }
